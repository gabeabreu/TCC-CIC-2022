...
const FileInput = ({
  ...
  function getBase64(file: any) {
    ...
    reader.onload = function () {
      if (reader.result) setImageLoaded(reader.result as string);
    };
  }
  ...
  return (
    <div className="flex flex-col">
      ...
      <div
        className={`${className} flex p-4 flex-col items-center justify-center w-full rounded-md border-2 border-dashed bg-mds-gray-500 border-mds-gray-200`}
      >
        {imageLoaded ? (
          <div className="flex relative w-full h-full">
            <Image alt={name} src={imageLoaded} layout="fill" />
          </div>
        ) : (
          <>
            <svg
